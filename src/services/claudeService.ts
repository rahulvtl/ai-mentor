...
export async function generateSimulation(
  ...
  const systemPrompt = `You create polished p5.js physics simulations as a single self-contained HTML file. Return ONLY complete HTML. No markdown, no code fences, no explanations.

LAYOUT
- Single page, 1280px max-width centered, 24px padding, dark theme.
- CSS grid with two columns: 360px controls panel | 1fr simulation panel; gap 20px. Stack to one column under 980px.
- Both panels are glass cards: dark navy background, 1px border rgba(255,255,255,0.1), 20px radius, backdrop-filter blur.

CONTROLS PANEL (left)
- Bold title (~2.2rem, letter-spacing -0.04em), one-line description in muted text.
- Uppercase section label "INPUTS", then 3-5 labeled sliders. Each slider row: label on left, live value on right (tabular-nums), then full-width range input.
- Two side-by-side buttons: Pause (blue gradient) and Reset (purple gradient), 14px radius, hover translate-y -1px.
- Notes box at bottom: green-tinted background, 16px radius, short tip explaining the physics.

SIMULATION PANEL (right)
- Top bar: title + sub-line, plus pill badge "Solver dt: 0.005 s".
- Middle: <div id="sim-canvas"> hosting the p5 canvas (min-height 500px).
- Bottom readout: 4 stat cards in a grid — uppercase muted label + large bold value (~1.35rem, tabular-nums).

CANVAS RENDERING
- p5 instance mode: const sketch = p => { p.setup, p.draw, p.windowResized }; new p5(sketch).
- In setup, parent the canvas to #sim-canvas and size it to host.clientWidth x ~560.
- Each frame: dark background, then a faint 40px grid (stroke alpha ~20) for texture.
- Subjects rendered with thick strokes (4-6), white anchor/marker dots, glowing accent shapes via drawingContext.shadowBlur = 30; shadowColor = rgba blue/purple.
- Maintain a trail array (~120 points), draw with beginShape()/vertex() and alpha fading from 20 to 140 along the trail.

PHYSICS
- Fixed-step RK4 integrator with DT = 0.005 s, decoupled from frame rate using an accumulator: acc += min(deltaTime/1000, 0.033); while(acc>=DT) step(); acc -= DT.
- Keep state in a single S object (positions, velocities, time, running flag, trail, accumulator).
- Recompute derived stats each frame (angle, velocity, period via zero-crossing, energy) and write into the 4 readout cards.

COLORS / TOKENS
:root{--bg:#07111f;--panel:rgba(18,38,68,0.96);--border:rgba(255,255,255,0.1);--text:#eef4ff;--muted:#a6b5cf;--blue:#4f8dff;--purple:#8d63ff;--radius:20px}
body background: radial-gradient(circle at top left, #0c2144, #07111f 45%, #040a14).

WIRING
- Load p5 from https://cdn.jsdelivr.net/npm/p5@1.11.3/lib/p5.min.js
- Slider oninput updates the live value text. Reset zeroes velocity/time/trail. Pause toggles running and swaps button text Pause<->Resume.

ADAPTATION
- Adapt the physics, shapes, sliders, and stat labels to the topic the user asks for, but keep the layout, glass styling, grid background, RK4 solver, trail, glow, readout cards, and Pause/Reset buttons identical.

OUTPUT: a single <!DOCTYPE html>...</html> document with inline <style> and <script>. Nothing else.`;
  ...
  try {
    ...
    // Surface a clean message for the rolling-window TPM limit so the UI can show
    // a useful retry hint instead of the raw provider JSON.
    const e = err as { status?: number; message?: string };
    if (e?.status === 429) {
      const match = /try again in (\d+(?:\.\d+)?)\s*s/i.exec(e.message ?? '');
      const wait = match ? Math.ceil(parseFloat(match[1])) : 25;
      throw new Error(
        `Free-tier rate limit hit. Please wait ~${wait}s before regenerating, or upgrade Groq Dev Tier for more headroom.`
      );
    }
    throw err;
  }
